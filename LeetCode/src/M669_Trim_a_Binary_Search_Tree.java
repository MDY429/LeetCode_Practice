/**
 * Given the root of a binary search tree and the lowest and highest boundaries
 * as low and high, trim the tree so that all its elements lies in [low, high].
 * Trimming the tree should not change the relative structure of the elements
 * that will remain in the tree (i.e., any node's descendant should remain a
 * descendant). It can be proven that there is a unique answer.
 * 
 * Return the root of the trimmed binary search tree. Note that the root may
 * change depending on the given bounds.
 * 
 * Example 1: Input: root = [1,0,2], low = 1, high = 2 Output: [1,null,2]
 * 
 * Example 2: Input: root = [3,0,4,null,2,null,null,1], low = 1, high = 3
 * Output: [3,2,null,1]
 * 
 * Example 3: Input: root = [1], low = 1, high = 2 Output: [1]
 * 
 * Example 4: Input: root = [1,null,2], low = 1, high = 3 Output: [1,null,2]
 * 
 * Example 5:Input: root = [1,null,2], low = 2, high = 4 Output: [2]
 * 
 * 
 * Constraints:
 * 1. The number of nodes in the tree in the range [1, 104]. 
 * 2. 0 <= Node.val <= 104
 * 3. The value of each node in the tree is unique.
 * 4. root is guaranteed to be a valid binary search tree.
 * 5. 0 <= low <= high <= 104
 */
public class M669_Trim_a_Binary_Search_Tree {

    /**
     * Using recursion to trim the node and connect the legal child node.
     * 
     * @param root The Binary Tree.
     * @param low  The lowest value.
     * @param high The highest value.
     * @return The Trim of binary tree.
     */
    public TreeNode trimBST(TreeNode root, int low, int high) {

        if (root == null) {
            return root;
        }

        if (root.val < low) {
            return trimBST(root.right, low, high);
        }
        if (root.val > high) {
            return trimBST(root.left, low, high);
        }

        root.left = trimBST(root.left, low, high);
        root.right = trimBST(root.right, low, high);

        return root;
    }

    public static void main(String[] args) {
        M669_Trim_a_Binary_Search_Tree m669 = new M669_Trim_a_Binary_Search_Tree();

        Integer[] arr = new Integer[] { 3, 1, 4, null, 2 };
        TreeNode root = TreeNode.build(arr);
        int low = 3;
        int high = 4;

        Integer[] a = new Integer[] { 3, null, 4 };
        TreeNode aa = TreeNode.build(a);
        System.out.println(aa.equals(m669.trimBST(root, low, high)));

    }
}